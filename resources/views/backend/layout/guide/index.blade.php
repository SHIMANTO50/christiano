            ...
            <div class="row mt-5">
                <div class="col-12">
                    <div class="text-end d-flex mx-3 justify-content-between align-items-center mb-4">
                        <h3>Guides</h3>
                        @can('create guide')
                            <a href="{{ route('guide.create') }}" class="btn btn-primary mb-3">Create a Guide</a>
                        @endcan
                    </div>
...
@push('script')
    ...
    <script>
        ...
        function deleteItem(id) {
            var url = '{{ route('guide.destroy', ':id') }}';
            $.ajax({
                type: "DELETE",
                url: url.replace(':id', id),
                success: function(resp) {
